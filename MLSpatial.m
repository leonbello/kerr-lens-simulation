...
f = ((2*pi*lens_aperture^2)/lambda)*1e-0;


phiKerr = @(Pt,W) exp((1i*Ikl.*Pt)./(W.^2)); % non-linear instantenous phase accumulated due to Kerr effect
...
for i=1:length(Et)
    ...
    M(:,:,i) =  distance(L/(N+1))*lensL(Feff11(i),f);
    ...
    M(:,:,i) = distance(L/(N+1))*lensL(Feff31(i),f);
    ...
    M(:,:,i) = distance(L/(N+1))*lensL(Feff41(i),f);
    ...
    M(:,:,i) = distance(L/(N+1))*distance(RM/2+deltaPlane-eps-L/2)*Mcur(RM)*distance(L1)*[1 0; 0 1]*distance(L1)*Mcur(RM)*distance(RM/2+deltaPlane-eps-L/2)*distance(L/(N+1))*lensL(Feff51(i),f);
    ...
    M(:,:,i) = distance(L/(N+1))*lensL(Feff52(i),f);
    ...
    M(:,:,i) = distance(L/(N+1))*lensL(Feff42(i),f);
    ...
    M(:,:,i) = distance(L/(N+1))*lensL(Feff32(i),f);
    ...
    M(:,:,i) = distance(L/(N+1))*lensL(Feff22(i),f);
    ...
    M(:,:,i) = distance(L/(N+1))*distance(V+deltaPoint-L/2)*lens(FM)*distance(L2)*[1 0; 0 1]*distance(L2)*lens(FM)*distance(V+deltaPoint-L/2)*distance(L/(N+1))*lensL(Feff12(i),f);
